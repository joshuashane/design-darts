...
function formatViewport(vp: { width: number; height: number; dpr: number }): string {
  return `${vp.width}×${vp.height}${vp.dpr !== 1 ? ` @${vp.dpr}x` : ''}`;
}

function groupByScreen(comments: NormalizedComment[]): Map<string, NormalizedComment[]> {
...
export function generateReport(comments: NormalizedComment[]): string {
  ...
  for (const [screenKey, screenComments] of groups) {
    ...
    for (const c of screenComments) {
      ...
      if (c.sourceLocation) lines.push(`**Source:** \`${c.sourceLocation}\``);
      if (c.textSnippet) lines.push(`**Element text:** "${c.textSnippet}"`);
      lines.push('');
